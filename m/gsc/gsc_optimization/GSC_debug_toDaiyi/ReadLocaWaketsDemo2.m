...
filepath = '/Users/daiyi02/Downloads/gsc_opt/';
...
fidTxtR = fopen([filepath filename],'r+','n');
WakNum = 0;
WakInfo = [];
tickRepFlag = 0;
while 1
    ...
    if contentLen >= 12
        if(sum(content(1:11)=='wake_status')==11)
            WakNum = WakNum + 1;
            WakInfo(WakNum, 11) = str2double(content(13:end));
            startFlag = 1;
        end        
    end
...
fidTxtW = fopen([filepath filename],'w+','n');
