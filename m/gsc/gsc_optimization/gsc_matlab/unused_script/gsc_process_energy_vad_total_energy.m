...
query_energy = zeros(1, MicNum);
query_energy_max = zeros(1, MicNum);
query_energy_id = zeros(1, MicNum);
...
%         return;
        ...
        atf_moments_plot_1(:, moment_length_index) = h_mat_plot_1(:, query_energy_id(1));
        atf_moments_plot_2(:, moment_length_index) = h_mat_plot_2(:, query_energy_id(2));
        atf_moments_plot_3(:, moment_length_index) = h_mat_plot_3(:, query_energy_id(3));
        atf_moments_plot_4(:, moment_length_index) = h_mat_plot_4(:, query_energy_id(4));
        atf_moments_plot_5(:, moment_length_index) = h_mat_plot_5(:, query_energy_id(5));
        atf_moments_plot_6(:, moment_length_index) = h_mat_plot_6(:, query_energy_id(6));
...
%         lp = (p - 1) * Frame_inc_atf + 1:(p - 1) * Frame_inc_atf + Frame_len_atf;
        ...
        query_energy(1) = sum(fv1(1:FreLen_atf));
        query_energy(2) = sum(fv2(1:FreLen_atf));
        query_energy(3) = sum(fv3(1:FreLen_atf));
        query_energy(4) = sum(fv4(1:FreLen_atf));
        query_energy(5) = sum(fv5(1:FreLen_atf));
        query_energy(6) = sum(fv6(1:FreLen_atf));
        
        for i = 1:MicNum
            if query_energy(i) > query_energy_max(i)
                query_energy_max(i) = query_energy(i);
                query_energy_id(i) = p;
            end
        end
