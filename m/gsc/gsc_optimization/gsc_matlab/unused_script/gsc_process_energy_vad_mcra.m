...
for i = 1 : MicNum
NoiseMcra(i).first  = 1;
NoiseMcra(i).len    = FreLen_atf;
% NoiseMcra.w    = 1;
% NoiseMcra.L    = 60;
NoiseMcra(i).alpha    = 0.9;
% NoiseMcra.frm_cnt    = 0;
NoiseMcra(i).S   = zeros(1, NoiseMcra(i).len);
NoiseMcra(i).Smin   = zeros(1, NoiseMcra(i).len);
% NoiseMcra.Stmp   = zeros(1, NoiseMcra.len);
NoiseMcra(i).Yprob   = zeros(1, NoiseMcra(i).len);
NoiseMcra(i).lamda_d   = zeros(1, NoiseMcra(i).len);
NoiseMcra(i).P = zeros(1, NoiseMcra(i).len);
% NoiseMcra.b   = zeros(1, 2*NoiseMcra.w+1);    %hanning window coefficients
end
query_prob_max = zeros(MicNum, FreLen_atf);
query_prob_id = zeros(MicNum, FreLen_atf);
%return;
...
%         query_energy_max = zeros(MicNum, FreLen_atf);
        ...
        for i = 1:FreLen_atf
            atf_moments_plot_1(i, moment_length_index) = h_mat_plot_1(i, query_prob_id(1, i));
            atf_moments_plot_2(i, moment_length_index) = h_mat_plot_2(i, query_prob_id(2, i));
            atf_moments_plot_3(i, moment_length_index) = h_mat_plot_3(i, query_prob_id(3, i));
            atf_moments_plot_4(i, moment_length_index) = h_mat_plot_4(i, query_prob_id(4, i));
            atf_moments_plot_5(i, moment_length_index) = h_mat_plot_5(i, query_prob_id(5, i));
            atf_moments_plot_6(i, moment_length_index) = h_mat_plot_6(i, query_prob_id(6, i));
        end
        for i = 1 : MicNum
            NoiseMcra(i).first  = 1;
            NoiseMcra(i).len    = FreLen_atf;
...
%         end
        ...
        % choose the greatest
        for i = 1:MicNum
            [noise, NoiseMcra(i), Sr] = mcra(query_energy(i, :), NoiseMcra(i));
%             [i, NoiseMcra(i).Yprob(100)]
            for j = 1:FreLen_atf
                if NoiseMcra(i).Yprob(j) >= query_prob_max(i, j)
                    query_prob_max(i, j) = NoiseMcra(i).Yprob(j);
                    query_prob_id(i, j) = p;
                end
            end              
        end
...
%return;
%% save result for atf
result_folder = './aj_185_3m_d2/';
for m = 1 : MicNum
    wavname_ori = ['rec_mic_ori_' num2str(m) '.wav'];
    wavname_construct = ['rec_mic_construct34_' num2str(m) '.wav'];
