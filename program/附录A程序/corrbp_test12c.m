function period=corrbp_test12c(y,fn,vseg,vsl,lmax,lmin,ThrC,tst_i1)
pn=size(y,2);
if pn~=fn, y=y'; end                      % 把y转为每列数据表示一帧语音信号
...
i=tst_i1;                                 % i=第tst_i1个元音主体
    ...
    for k=1 : ixd                         
        ...
        [Sv,Kv]=findmaxesm3(ru,lmax,lmin);% 获取3个极大值的数值和位置
        lkv=length(Kv);
        Ptk(1:lkv,k)=Kv';                 % 把每帧3个极大值位置存放在Ptk数组中
    end
    Kal=Ptk(1,:);
    figure(51);clf
    ...
    if thegma>5, 
        ...
        Ptemp(1,(Ptk(1,:)<mt1 & Ptk(1,:)>mt2))=1;% 检查各组数据有否在置信区内
        Ptemp(2,(Ptk(2,:)<mt1 & Ptk(2,:)>mt2))=1;
        Ptemp(3,(Ptk(3,:)<mt1 & Ptk(3,:)>mt2))=1;
    
        Pam=zeros(1,ixd);                 % 在Pam中存放置信区内大的数值
        for k=1 : ixd
            if Ptemp(1,k)==1
        ...
        save dpstp_tmpdata1.mat Ptk Pamtmp pindex c1 meanx mt1 mt2
        period=[]; return
        
        if length(pindex)~=ixd
            bpseg=findSegment(pindex);        % 计算置信区间内的数据分段信息
            bpl=length(bpseg);                % 置信区间内的数据分成几段 
            bdb=bpseg(1).begin;               % 置信区间内笫一段的开始位置
            if bdb~=1                         % 如果置信区间内笫一段开始位置不为1
                Ptb=Pamtmp(bdb);              % 置信区间内笫一段开始位置的基音周期
                Ptbp=Pamtmp(bdb+1);
            ...
            Pte=Pamtmp(bde);
            Pten=Pamtmp(bde-1);
            if bde~=ixd                 % 如果置信区间内最后一段开始位置不为ixd
